...
public:
   ...
   */
  LCQProblem(int _nV, int _nC, int _nComp);
   ...
   * @param _ubL The upper bounds associated to the complementarity matrix `_L`. A `NULL` pointer can be passed if no
   * upper bounds exist.
   * @param _lbR The lower bounds associated to the complementarity matrix `_R`. A `NULL` leads to zero boudns.
   * @param _ubR The upper bounds associated to the complementarity matrix `_R`. A `NULL` pointer can be passed if no
   * upper bounds exist.
   * @param _A The constraint matrix. A `NULL` pointer can be passed if no linear constraints exist.
   * @param _lbA The lower bounds associated to the constraint matrix `_A`. A `NULL` pointer can be passed if no lower
   * bounds exist.
   * @param _ubA The upper bounds associated to the constraint matrix `_A`. A `NULL` pointer can be passed if no upper
   * bounds exist.
   ...
   * @param _x0 The initial guess for the optimal primal solution vector. If a `NULL` pointer is passed the zero vector
   * is used.
   * @param _y0 The initial guess for the optimal dual solution vector. If a `NULL` pointer is passed, then the
   * initialization depends on the subsolver and its options.
   *
   * @returns SUCCESSFUL_RETURN if a solution is found. Otherwise the return value will indicate an occured error.
   */
  ReturnValue loadLCQP(const double* const _Q, const double* const _g, const double* const _L, const double* const _R,
                       const double* const lbL = 0, const double* const ubL = 0, const double* const lbR = 0,
                       const double* const ubR = 0, const double* const _A = 0, const double* const _lbA = 0,
                       const double* const _ubA = 0, const double* const _lb = 0, const double* const _ub = 0,
                       const double* const _x0 = 0, const double* const _y0 = 0);
   ...
   * @param ubL_file The upper bounds associated to the complementarity matrix `_L`. A `NULL` pointer can be passed if
   ...
   * @param ubR_file The upper bounds associated to the complementarity matrix `_R`. A `NULL` pointer can be passed if
   * no upper bounds exist.
   * @param A_file The constraint matrix. A `NULL` pointer can be passed if no linear constraints exist.
   * @param lbA_file The lower bounds associated to the constraint matrix `_A`. A `NULL` pointer can be passed if no
   * lower bounds exist.
   * @param ubA_file The upper bounds associated to the constraint matrix `_A`. A `NULL` pointer can be passed if no
   * upper bounds exist.
   ...
   * @param x0_file The initial guess for the optimal primal solution vector. If a `NULL` pointer is passed the zero
   * vector is used.
   * @param y0_file The initial guess for the optimal dual solution vector. If a `NULL` pointer is passed, then the
   * initialization depends on the subsolver and its options.
   *
   * @returns SUCCESSFUL_RETURN if a solution is found. Otherwise the return value will indicate an occured error.
   */
  ReturnValue loadLCQP(const char* const Q_file, const char* const g_file, const char* const L_file,
                       const char* const R_file, const char* const lbL = 0, const char* const ubL = 0,
                       const char* const lbR = 0, const char* const ubR = 0, const char* const A_file = 0,
                       const char* const lbA_file = 0, const char* const ubA_file = 0, const char* const lb_file = 0,
                       const char* const ub_file = 0, const char* const x0_file = 0, const char* const y0_file = 0);
   ...
   * @returns SUCCESSFUL_RETURN if a solution is found. Otherwise the return value will indicate an occured error.
   */
  ReturnValue loadLCQP(const csc* const _Q, const double* const _g, const csc* const _L, const csc* const _R,
                       const double* const lbL = 0, const double* const ubL = 0, const double* const lbR = 0,
                       const double* const ubR = 0, const csc* const _A = 0, const double* const _lbA = 0,
                       const double* const _ubA = 0, const double* const _lb = 0, const double* const _ub = 0,
                       const double* const _x0 = 0, const double* const _y0 = 0);
   ...
   *
   * @returns If the problem was solved successfully the stationarity type is passed. Else PROBLEM_NOT_SOLVED is
   * returned.
   */
...
protected:
  ...
  ReturnValue setComplementarityBounds(const double* const lbL_new, const double* const ubL_new,
                                       const double* const lbR_new, const double* const ubR_new);
   ...
   */
  inline ReturnValue setLB(int number, double value);
   ...
   * @param value New value for entry of lower bound vector.
   */
  inline ReturnValue setUB(int number, double value);
   ...
   */
  ReturnValue setConstraints(const double* const L_new, const double* const R_new, const double* const A_new,
   ...
   */
  ReturnValue setConstraints(const csc* const L_new, const csc* const R_new, const csc* const A_new,
                             const double* const lbA, const double* const ubA);
