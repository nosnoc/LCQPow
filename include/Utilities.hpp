...
enum ReturnValue
{
  ...
  = 102, /**< Invalid complementarity tolerance. Must be no smaller than machine precision. */
  ...
  = 105,                              /**< Invalid stationarity tolerance. Must be no smaller than machine precision. */
  ...
  INVALID_OSQP_BOX_CONSTRAINTS = 110, /**< Invalid constraints passed to OSQP solver: This solver does not handle box
                                         constraints, please pass them through linear constraints. */
  INVALID_TOTAL_ITER_COUNT
  = 111, /**< Invalid total number of iterations delta passed to output statistics (must be non-negative integer). */
  INVALID_TOTAL_OUTER_ITER = 112, /**< Invalid total number of outer iterations delta passed to output statistics (must
                                     be non-negative integer). */
  IVALID_SUBPROBLEM_ITER = 113,   /**< Invalid total number of subproblem solver iterates delta passed to output
                                     statistics (must be non-negative integer). */
  ...
  = 115, /**< Invalid integer to be parsed to print level passed (must be in range of enum). */
  INVALID_OBJECTIVE_LINEAR_TERM
  = 116, /**< Invalid objective linear term passed (must be a double array of length n). */
  INVALID_CONSTRAINT_MATRIX
  = 117, /**< Invalid constraint matrix passed (matrix was null pointer but number of constraints is positive). */
  INVALID_COMPLEMENTARITY_MATRIX = 118, /**< Invalid complementarity matrix passed (can not be null pointer). */
  INVALID_ETA_VALUE = 119, /**< Invalid etaDynamicPenalty value, which describes the fraction of loss required for
                              complementarity progress (must be in (0,1)). */
  ...
  = 205,                        /**< Failed to switch to sparse mode (a to be created sparse matrix was nullpointer). */
  ...
  DENSE_SPARSE_MISSMATCH
  = 402, /**< Solver was initialized with dense (sparse) matrices but a sparse (dense) method was chosen. */
};
...
public:
  ...
  /** C = A'*B **/
  static void TransponsedMatrixMultiplication(const double* const A, const double* const B, double* C, int m, int n,
                                              int p);
  ...
  /** C += A'*B **/
  static void AddTransponsedMatrixMultiplication(const double* const A, const double* const B, double* C, int m, int n,
                                                 int p);
  ...
  /** d = A*b + c **/
  static void AffineLinearTransformation(const double alpha, const double* const A, const double* const b,
                                         const double* const c, double* d, int m, int n);

  /** d = A*b + c **/
  static void AffineLinearTransformation(const double alpha, const csc* const S, const double* const b,
                                         const double* const c, double* d, int m);

  /** C = alpha*A + beta*B **/
  static void WeightedMatrixAdd(const double alpha, const double* const A, const double beta, const double* const B,
                                double* C, int m, int n);

  /** c = alpha*a + beta*b **/
  static void WeightedVectorAdd(const double alpha, const double* const a, const double beta, const double* const b,
                                double* c, int m);
  ...
  template <typename PtrType> static bool isNullPtr(PtrType ptr) { return (ptr == 0 || ptr == nullptr); }

  /** Checks if the ptr is not null. */
  template <typename PtrType> static bool isNotNullPtr(PtrType ptr) { return (ptr != NULL && ptr != nullptr); }
